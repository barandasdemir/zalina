...
module.exports = {
    ...
    async insertProduct(productInfo) {
        ...
        /* const productTypeId = await db('productTypes').
             where('name', productInfo.product_type).select('id');
         console.log(productTypeId);*/
        const resultid = [];
        await db('products').insert({ name: productInfo.product_title, description: productInfo.product_desc, productType: Number.parseInt(productInfo.product_type), price: productInfo.product_price, productCode: productInfo.product_code })
            .then(async pid => {
                await db('productColors').insert({ name: productInfo.product_color, product: pid[0], productCode: productInfo.product_code });
                const proid = pid[0];
                const sizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL'];
                for (let i = 0; i < sizes.length; i++) {
                    await db('sizes').insert({ name: sizes[i], stock: Object.values(productInfo)[i] })
                        .then(async res => {
                            await db('productSizes').insert({ product: proid, size: res[0] });
                        })
                }
                resultid.push(proid);
