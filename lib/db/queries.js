...
module.exports = {
    ...
    async insertProduct(productInfo) {
        ...
        const sizes = await db.raw(`select id, name from sizes where type = ${types.findIndex(type => type === productInfo.stock_type)}`);
        console.log(sizes);
        const stocks = [];
        for (let i = 0; i < sizes.length; i++) {
            stocks.push({
                product: pid,
                size: await this.getValue("id", "sizes", "name", sizes[i]),
                stock: Number.parseInt(productInfo[sizes[i]])
            });
        }
        console.log(stocks);
